...
function addPicture() {
  ...
  var author = $("[name = author")[0].value;
  var dateCreate = $("[name = dateCreate")[0].value;
  var country = $("[name = country")[0].value;
  var beginPrice = $("[name = beginPrice")[0].value;
  var minStep = $("[name = minStep")[0].value;
  var maxStep = $("[name = maxStep")[0].value;
  var description = $("[name = description")[0].value;
  var srcref = $("[name = srcref")[0].value; // если что-то не заполнено, не отправялем

  if (dateCreate == "" || title == "" || author == "" || country == "" || beginPrice == "" || maxStep == "" || maxStep == "") {
    alert("Незаполенные данные!");
    return;
  } // создаем тело запроса


  var body = 'title=' + encodeURIComponent(title) + '&author=' + encodeURIComponent(author) + '&dateCreate=' + encodeURIComponent(dateCreate) + '&country=' + encodeURIComponent(country) + '&beginPrice=' + encodeURIComponent(beginPrice) + '&minStep=' + encodeURIComponent(minStep) + '&maxStep=' + encodeURIComponent(maxStep) + '&description=' + encodeURIComponent(description) + '&srcref=' + encodeURIComponent(srcref);
  console.log("Body:", body); // console.log("Title of new book to server: ", title);
  ...
  $.post("/pictures", body, function (data) {
...
function addNewNodePictureToTable(picture) {
  var htmlRow = "<tr onClick=getPictureFromServer(" + picture.id + ") id=picture" + picture.id + ">";
...
function openModalWindowToChange(picture) {
  ...
  var dialog = document.getElementById("modal-window-change"); // let currPicture = JSON.parse(picture);

  currPictureID = picture.id; // let titleInput = document.getElementById("title-change");
...
function changePicture(pictureFields) {
  var body = 'id=' + encodeURIComponent(pictureFields[0]) + '&title=' + encodeURIComponent(pictureFields[1]) + '&author=' + encodeURIComponent(pictureFields[2]) + '&dateCreate=' + encodeURIComponent(pictureFields[3]) + '&country=' + encodeURIComponent(pictureFields[4]) + '&beginPrice=' + encodeURIComponent(pictureFields[5]) + '&minStep=' + encodeURIComponent(pictureFields[6]) + '&maxStep=' + encodeURIComponent(pictureFields[7]) + '&description=' + encodeURIComponent(pictureFields[8]) + '&srcref=' + encodeURIComponent(pictureFields[9]);
  ...
  $.ajax({
    ...
    success: function success(response) {
      var changedPicture = JSON.parse(response);
      console.log(changedPicture);
      $('#picture' + currPictureID).children().first().text("" + changedPicture.title).next().text("" + changedPicture.author).next().text("" + changedPicture.dateCreate).next().text("" + changedPicture.country).next().text("" + changedPicture.beginPrice);
    }
  });
}
...
function openImage() {
  ...
  var src = "images/" + $('#modal-window-change-form >.div-srcref > input').val();
